...
//
//			@Override
//			protected LabelingCell<String> compute(final ImgPlusValue<BitType> thinnedValue,
//					final ImgPlusValue<BitType> sourceValue) throws Exception {
//
//				if (thinnedValue.getDimensions().length != 2) {
//					throw new IllegalArgumentException("Only two dimensions are supported since now");
//				}
//
//				final ImgPlus<BitType> thinned = thinnedValue.getImgPlus();
//				final ImgPlus<BitType> source = sourceValue.getImgPlus();
//
//				final RandomAccess<BitType> thinnedAccess = thinned.randomAccess();
//
//				final IterableInterval<Neighborhood<BitType>> neighborhoods = new RectangleShape(1, true)
//						.neighborhoods(Views.interval(Views.extendValue(thinned, new BitType(false)), thinned));
//
//				final Cursor<Neighborhood<BitType>> neighborhoodCursor = neighborhoods.cursor();
//
...
//						continue;
//
//					final Cursor<BitType> oneNeighborhood = neighborhoodCursor.get().cursor();
//
//					int ctr = 0;
...
//						new ArrayImgFactory<UnsignedByteType>().create(source, new UnsignedByteType()));
//
//				final Sum<BitType, DoubleType> sumOp = new Sum<BitType, DoubleType>();
//
//				final RandomAccess<Neighborhood<BitType>> ra = new RectangleShape(2, true)
//						.neighborhoodsRandomAccessibleSafe(
//								Views.interval(Views.extendValue(source, new BitType(false)), source))
//						.randomAccess(source);
//
//				ra.setPosition(posses[0]);
//
//				final double tmp = sumOp.compute(ra.get().cursor(), new DoubleType()).get();
//
...
//						new DefaultLabelingMetadata(resAccess.numDimensions(), new DefaultLabelingColorTable()));
//			}
//
//			@Override
